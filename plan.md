...
## Phase 6 — 키오스크 유휴 타임아웃

- 60초간 입력 없으면 "계속하시겠습니까?" 확인 모달(예/아니오) 표시.
- 무응답 또는 "아니오" 선택 시 `/`(홈)로 리셋 + `openChatPane(idlePageUrl)`으로 아래쪽 채팅 탭도 대기 화면으로 되돌림.
- 타이머는 클릭/터치/키 입력 등 모든 상호작용에서 리셋.

## Phase 7 — 라이트 테마 + 토글

- 다크 테마가 안정화된 후, 같은 컬러 토큰 체계로 라이트 테마 값을 추가.
- 상단 메뉴에 테마 토글 버튼 추가.

## Phase 8 — 아키텍처 화면 (`/architecture`) — 최후순위

- 시안 A: 위→아래로 흐르는 정적 다이어그램(데이터 → Gemini Enterprise → 결과물), 3D 드래그 없음.
- 시안 B: 다이어그램 없이 아이콘+텍스트 카드 3~4개.
- 두 시안을 실제로 구현해서 나란히 보여주고, 사용자가 직접 고른 뒤 하나만 남긴다.
